...
export default function Sidebar({ current, onLogout, sosCount, alertCount }: { current: Page; onLogout: () => void; sosCount: number; alertCount: number }) {
  ...
  return <aside className="design-sidebar">
    ...
    <nav aria-label="Main navigation">{navigation.filter(([id]) => id !== "users" || user.role === "admin").map(([id,label]) => {
      const count = id === "alerts" ? alertCount : id === "sos" ? sosCount : 0;

...
export default function Header({current}:{current:Page}) {
  ...
  const monitoringPage = ["dashboard","stations","analytics","health"].includes(current);
  ...
  return <header className="design-header"><h1>{titles[current]}</h1><div className="header-meta">{current === "alerts" ? <span className="subtle-note" title="Persistent sensor episodes; device/simulator origin is not recorded per episode">SENSOR EPISODES</span> : <span className="connection-label" title={monitoringPage ? modeTitle : "Connected account service"}><i/>{current === "sos" ? "PRACTICE" : monitoringPage ? modeLabel : "ONLINE"}</span>}<time dateTime={now?.toISOString()}>{date ?? "—"}<b>·</b>{time ?? "—"}</time></div></header>;
}
